...
**Long-only top-k cross-sectional momentum (`traderstack-xs-topk`, #140).** A **portfolio bar**, not the per-asset #96+A+B+C bar: #117 scored each of three names separately and its own rules text rejected equal-weight portfolio metrics for that bar, so a top-k basket needs a new, pre-registered unit of evaluation — the basket equity curve against an equal-weight buy-and-hold control of the same point-in-time universe. Universe: the dated Kraken `AssetPairs` listing (online USD pairs; frozen exclusion list; survivorship caveat in the header), then monthly top-20 by trailing 30-day median dollar volume using only bars before each snapshot. Frozen grid: N in {21, 63, 126} with a 7-day skip, k in {3, 5}, equal or inverse-vol weights, weekly Monday-UTC rebalance decided on close t and filled at the next open; a name missing a bar is skipped for that week, fewer than 10 rankable names is a flat week. Costs are printed at research 10+5 bps and at the Kraken tier-1 pilot taker cost (80+5 bps); the bar and the ranking use the pilot print, with one-way turnover per year and fee drag on every cell. Bar (frozen): in every covered era (2016–2019 / 2020–2022 / 2022–2024 / 2024–2026, ≥ 365 bars; a local constant until #135) net return > 0, net Sharpe > 0 and net excess over `ew_bh_universe` > 0, plus the print's last-20% holdout excess > 0 — a family that only works in one era fails. Dual print: two independent covered cells (distinct venues or non-overlapping eras) pass and none fail, so a single-venue single-era print cannot promote. Ranking key (frozen): `mean_era_excess_vs_ew_bh_among_dual_print_passers`, tie-break `candidate_id`. Deflated Sharpe and PBO are deferred to #135 and printed as `not_computed_pending_135`. The control cannot promote. `RiskEngine` limits (`MAX_OPEN_POSITIONS`, `MAX_POSITION_PCT`, `MAX_GROSS_EXPOSURE_PCT`, `MVP_ASSETS`) are documented, not widened. Never flips `PAPER_PROMOTE_*`; adds no Settings field. Empty dual-print set is success. Report: `docs/artifacts/strategy-search/xs-topk.md`.
