...
**Harder honesty gates (`traderstack-harder-gates`).** Same Kraken daily 720-bar cap and balanced catalog, with three gates pre-registered before scoring: **A** magnitude (`min/max` BTC vs ETH holdout excess ≥ 0.25, both signs > 0); **B** three contiguous 240-bar windows requiring BTC **and** ETH WF total > 0 in at least 2 of 3 (each window uses train=180/test=60 and no in-window holdout); **C** 2× fees (20+10 bps) must still clear #96 balanced signs. Yahoo remains A/B only. Ranking is still pre-registered top-1; a more balanced ADX/SMA #2 cannot skip a failing `ema_9_21`. Combined promote = #96 **and** A **and** B **and** C **and** top-1. Does not flip `PAPER_PROMOTE_EMA_9_21`. An honest FAIL is success. Report: `docs/artifacts/strategy-search/magnitude-multiwindow-report.md`.
