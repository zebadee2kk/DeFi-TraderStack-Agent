...
Pre-registration only (no PnL claim): `docs/artifacts/strategy-search/xs-topk-archive-dual-print-recipe.md` freezes how to feed #147 candle archives into `traderstack-xs-topk --candles-dir` for a second covered cell. Never flips `PAPER_PROMOTE_*`. Empty dual-print set is success.
...
| `traderstack-polymarket-crypto-eval` | Fee-aware evaluation of the #142 wedge tape against `always_hold` and `fade_the_mid`. Requires operator tape + settlements (or `--empty-live`). Dual independent prints required before anyone may talk about promotion. Writes `docs/artifacts/strategy-search/polymarket-crypto-wedge-eval.md`. Never flips `PAPER_PROMOTE_*`. Empty / negative is success. No CLOB/Deribit orders. |
...
## Polymarket crypto wedge evaluation (#142 slice-2)

`traderstack-polymarket-crypto-eval` is report-only. Pre-register recipe first (`docs/artifacts/strategy-search/polymarket-crypto-wedge-eval-recipe.md`). Use `--empty-live` when no settled PIT tape exists; supply `--tape` and `--settlements` only when operator-collected (skip-not-invent). Never flips `PAPER_PROMOTE_*`. Empty / negative is success.

```bash
TRADING_MODE=paper .venv/bin/traderstack-polymarket-crypto-eval --empty-live
```
