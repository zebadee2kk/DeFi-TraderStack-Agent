...
| `insufficient_book_depth` | The notional resting within `RISK_MIN_DEPTH_BPS` of mid on the side the order must **take** (asks for a BUY, bids for a SELL) is below `RISK_MIN_DEPTH_MULTIPLE` x the requested notional (#61). The spread gate above bounds the *price* at the top of book; this bounds the *size* behind it, so a clip sized off NAV cannot fill well through a thin book. Risk-reducing exits are exempt. | Usually a genuinely thin book: reduce size, or wait. Persistent rejections on a liquid pair mean the book feed is stale or partial — check `KRAKEN_BOOK_ENABLED` and `book_error` on the cycle. |
| `book_depth_unavailable` | `RISK_REQUIRE_BOOK_DEPTH=true` and no book snapshot reached the engine (provider disabled, or it errored this cycle). Absent depth is treated as *no information*, never as "deep enough". | Expected when the book provider is down and you asked for depth to be mandatory. Set `RISK_REQUIRE_BOOK_DEPTH=false` to fall back to the spread gate alone, or fix the feed. On the Robinhood Chain path there is no book at all, so leave it false. |
