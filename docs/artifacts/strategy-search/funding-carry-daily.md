...
Generated: 2026-09-12T18:04:18.457835+00:00
Print kind: **dual_print**. interval=`1d`; primary_venue=`hyperliquid`; second_venue=`okx`; hard_gates_available=`false`; can_promote=`false`; basis_status=`skipped`; paper_path_ready=`false`; `keep_flag_false=true`.
...
Hard gates: UNAVAILABLE: need interval=1d and >=720 aligned bars on each participating venue; got interval=1d, primary_aligned_bars=720, second_aligned_bars=96.
Funding resample: UTC `1d` sums; empty days omitted.
Aligned bars: primary=720, second=96.
...
Pre-registered funding/carry search (frozen before any Kraken or funding-REST pull). Funding-z threshold voters and spot overlays instantiate only when an aligned funding series is supplied — never zero-filled. Hedged carry is a research model of cash-and-carry: received |funding| minus two-leg (spot+perp) fees on each flip; perp-spot basis is not invented and is not in the PnL. Dual-print requires two independent funding venues (e.g. OKX and Hyperliquid) each covering BTC and ETH. A second candle venue without a second funding tape is not dual-print. Same-venue prefix/suffix is not independent. Hard gates (#96+A+B+C) need 720 aligned daily bars on each funding venue; a ~90d OKX tape cannot unlock them (including after UTC-day resample). Daily evaluation sums settlements per UTC day and omits empty days — never zero-filled. Perp-spot basis is skipped unless a PIT mark−index / perp-mid−spot-mid series is supplied; funding premium and last-trade are not basis. Hedged carry is not paper-spot executable (no paper perp simulator / hedge book). Absent two venues this run is SINGLE-PRINT and cannot promote. A pin requires dual-print + hard gates + PIT basis + a paper-executable path. PAPER_PROMOTE_* stays default false. No live. Empty search is success. Funding tapes were resampled to UTC 1d sums (empty days omitted, not zero-filled). Hard gates (#96+A+B+C) are UNAVAILABLE on this overlap (interval=1d, primary_aligned_bars=720, second_aligned_bars=96; need 1d and >=720 on each participating venue). Primary carry hard gates: carry_hedged_sign daily hard gates: #96=true A=true (ratio=0.8741288051923678) B=true (3/3) C=true combined=true. w1: BTC=0.015155537638074312 ETH=0.01379697913744904 pass=true; w2: BTC=0.01312875992260909 ETH=0.015352042577623903 pass=true; w3: BTC=0.013714281654697125 ETH=0.014612916847969837 pass=true. Second carry hard gates: UNAVAILABLE: need >=720 daily funding prints on BTC and ETH; shortest tape has 97. Basis skipped (no PIT series). Paper-executable path is missing (Kraken spot fills only; no paper perp / hedge book). Do not add a Settings pin. can_promote stays false unless dual-print AND hard gates AND PIT basis AND a paper-executable path all clear.
...
- `second_funding:BTC/USD` **ok**: resampled 290 prints → 97 UTC daily sums (empty days omitted, not zero-filled) (source=funding_carry.resample_daily, points=97)
- `second_funding:ETH/USD` **ok**: resampled 290 prints → 97 UTC daily sums (empty days omitted, not zero-filled) (source=funding_carry.resample_daily, points=97)
...
| ---: | --- | --- | ---: | ---: | ---: | :---: |
| — | `ema_9_21_funding_agree` | funding_z | n/a | n/a | n/a | no |
| — | `funding_z_fade_1_0` | funding_z | n/a | n/a | n/a | no |
| — | `funding_z_fade_1_5` | funding_z | n/a | n/a | n/a | no |
| — | `funding_z_fade_2_0` | funding_z | n/a | n/a | n/a | no |
| — | `funding_z_follow_1_0` | funding_z | n/a | n/a | n/a | no |
| — | `funding_z_follow_1_5` | funding_z | n/a | n/a | n/a | no |
| — | `funding_z_follow_2_0` | funding_z | n/a | n/a | n/a | no |
| — | `ma_cross_10_30` | ma_cross | n/a | n/a | n/a | no |
| — | `momentum_12_funding_agree` | funding_z | n/a | n/a | n/a | no |

| id | WF total | holdout total | full-sample | eligible |
| --- | ---: | ---: | ---: | :---: |
| `carry_hedged_sign` | n/a | +0.31% | +0.94% | no |
| `carry_hedged_abs_1bp` | n/a | -0.93% | -5.82% | no |
| `carry_hedged_abs_3bp` | n/a | -0.57% | -1.38% | no |
| `carry_hedged_z_1_5` | n/a | -0.87% | -5.62% | no |
...
- Second (`okx`): available=`false`; UNAVAILABLE: need >=720 daily funding prints on BTC and ETH; shortest tape has 97

## Dual-print passers

Dual-print was eligible (two independent funding venues) but no name cleared the fee-aware bar on **both** prints. Empty set is success.
