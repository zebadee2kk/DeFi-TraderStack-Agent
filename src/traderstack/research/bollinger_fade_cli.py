"""`traderstack-bollinger-fade`: Bollinger band-fade dual-print.

Scores the frozen fade-to-inside / long-only fade / squeeze-breakout
contrast catalog on the Kraken primary 720-bar daily window and the
#102 Binance.US older-720. A name must combined-pass **both** prints.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bollinger band-fade / mean-reversion dual-print: fade "
            "to inside the bands (period×k in {20x2, 20x2.5, 40x2}), "
            "long-only fade, and a squeeze-breakout contrast. Must "
            "clear Kraken primary harder combined gates (#96+A+B+C "
            "on BTC+ETH; SOL reported) AND the #102 Binance.US "
            f"older-720 combined gates. Ranking key (frozen): "
            f"{RANKING_KEY}. Does not flip PAPER_PROMOTE_* flags. "
            "An empty dual-print set is success. Not an EMA, residual, "
            "XS, Donchian, TSMOM, or mean_reversion_* reprint. No live."
        )
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/bollinger_fade.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/bollinger-fade.md"),
    )
...
def run(
    ...
    *,
    ...
    report = run_bollinger_fade_search(
        kraken_histories,
        binance_histories,
        fee_bps=fee_bps,
    ...
    markdown = render_bollinger_fade_markdown(report)
