"""`traderstack-calendar-seasonality`: calendar seasonality dual-print.

Scores the frozen UTC day-of-week / month-of-year / turn-of-month
catalog on the Kraken primary 720-bar daily window and the #102
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Calendar seasonality dual-print: UTC day-of-week "
            "long-only (Mon / Fri / Mon+Fri), skip-weekend "
            "(long Mon–Fri, flat Sat/Sun), month-of-year "
            "(Q4 / Jan / Nov+Dec), and turn-of-month last 3 / "
            "first 3 UTC calendar days. Must clear Kraken "
            "primary harder combined gates (#96+A+B+C on BTC+ETH; "
            "SOL reported) AND the #102 Binance.US older-720 "
            f"combined gates. Ranking key (frozen): {RANKING_KEY}. "
            "Does not flip PAPER_PROMOTE_* flags. An empty "
            "dual-print set is success. Not an EMA, residual, XS, "
            "Donchian, TSMOM, or Bollinger reprint. No live."
        )
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/calendar_seasonality.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/calendar-seasonality.md"),
    )
...
def run(
    ...
    *,
    ...
    markdown = render_calendar_seasonality_markdown(report)
