"""Donchian / channel-breakout dual-print search (BTC+ETH; SOL optional).

Pre-registered before any live pull (do not retune after seeing PnL).

Treatment (frozen): on each asset, compare close[t] to the **prior**
N-day channel — max(high) and min(low) of bars [t-N, t), never
including bar t's high/low in the breakout level. Long-only names
enter long when close[t] > prior high and exit to flat when
close[t] < prior low. Long/short names go short on the opposite
band. Between the bands the previous side is held (start flat).
ATR-buffered long-only names use the same N for the channel and a
frozen Wilder ATR(14) computed on bars through t-1 (multiplier 1.0):
enter long when close[t] > prior high + ATR; exit flat when
close[t] < prior low − ATR.

N in {20, 55, 100} for unbuffered books; ATR books are N in {20, 55}
only. Do not grow this list after seeing PnL.

Fill at t+1 open (same convention as relative_value / xs_momentum).
Each asset is scored on its own OHLC. A missing/short series is
...
DONCHIAN_LOOKBACKS: tuple[int, ...] = (20, 55, 100)
DONCHIAN_ATR_LOOKBACKS: tuple[int, ...] = (20, 55)
DONCHIAN_ATR_PERIOD = 14
DONCHIAN_ATR_MULTIPLIER = 1.0
# (candidate_id, channel_n, long_short, use_atr)
DONCHIAN_CATALOG: tuple[tuple[str, int, bool, bool], ...] = (
    ("donchian_lo_20", 20, False, False),
    ("donchian_lo_55", 55, False, False),
    ("donchian_lo_100", 100, False, False),
    ("donchian_ls_20", 20, True, False),
    ("donchian_ls_55", 55, True, False),
    ("donchian_ls_100", 100, True, False),
    ("donchian_lo_atr_20", 20, False, True),
    ("donchian_lo_atr_55", 55, False, True),
)
DONCHIAN_IDS: tuple[str, ...] = tuple(item[0] for item in DONCHIAN_CATALOG)
CONTROL_ID = "ma_cross_10_30"
CONTROL_IDS: frozenset[str] = frozenset({CONTROL_ID})
CORE_IDS: tuple[str, ...] = DONCHIAN_IDS + (CONTROL_ID,)
...
DONCHIAN_RULES = (
    "Pre-registered Donchian / channel-breakout dual-print bar "
    "(frozen before any Kraken or Binance.US score). Treatment: "
    "compare close[t] to the prior N-day channel (max high / min low "
    "of bars [t-N, t); bar t's high/low never set the breakout "
    f"level). Exit rule `{EXIT_RULE}`: long-only exits to flat when "
    "close[t] < prior N-day low; long/short goes short on that band. "
    "Between the bands the previous side is held (start flat). N in "
    "{20, 55, 100}. ATR-buffered long-only names (`donchian_lo_atr_"
    f"{{20,55}}`) use the same N plus frozen Wilder ATR("
    f"{DONCHIAN_ATR_PERIOD}) × {DONCHIAN_ATR_MULTIPLIER:g} computed "
    "through t-1: enter when close > prior high + ATR, exit flat "
    "when close < prior low − ATR. Decision at bar t; fill at t+1 "
    "open. Each asset uses its own OHLC; a missing/short series is "
    "skipped, never zero-filled. "
    ...
    "BTC−ETH residual reprint, not cross-sectional momentum, and "
    "not a carry/basis family."
)

DONCHIAN_CATALOG_NOTE = (
    "Frozen catalog (K=9): long-only channel breakout "
    "(`donchian_lo_{20,55,100}`), long/short symmetric "
    "(`donchian_ls_{20,55,100}`), ATR-buffered long-only "
    f"(`donchian_lo_atr_{{20,55}}`; ATR period {DONCHIAN_ATR_PERIOD}, "
    f"multiplier {DONCHIAN_ATR_MULTIPLIER:g}), plus informational "
    "control ma_cross_10_30 (cannot promote). Do not grow this list "
    "after seeing PnL. Channel and ATR are built from venue-local "
    "OHLC; a missing series is skipped, never zero-filled. "
...
def wilder_atr(candles: tuple[Candle, ...], period: int = DONCHIAN_ATR_PERIOD) -> float | None:
    """Wilder ATR on the supplied bars only. Caller must exclude bar t."""
    if period <= 1 or len(candles) < period + 1:
        return None
    true_ranges = [
        _true_range(candles[index], candles[index - 1]) for index in range(1, len(candles))
    ]
...
def prior_channel(
    ...
    """Prior N-bar high/low ending at ``index`` exclusive. No look-ahead."""
    if channel_n <= 0 or index < channel_n:
        return None
    window = candles[index - channel_n : index]
    if len(window) < channel_n:
        return None
...
def donchian_position_series(
    ...
    *,
    ...
    atr_multiplier: float = DONCHIAN_ATR_MULTIPLIER,
) -> tuple[tuple[datetime, float], ...]:
    """Point-in-time Donchian positions. Skip-not-invent; no t high/low in level."""
    if channel_n <= 0 or len(candles) < channel_n + 1:
        return ()
    out: list[tuple[datetime, float]] = []
    position = 0.0
    for index in range(channel_n, len(candles)):
        ...
        buffer = 0.0
        if use_atr:
            atr = wilder_atr(candles[:index], atr_period)
            if atr is None:
                continue
            buffer = atr_multiplier * atr
        close = candles[index].close
        if close > prior_high + buffer:
            position = 1.0
        elif close < prior_low - buffer:
            position = -1.0 if long_short else 0.0
...
def donchian_signals(
    ...
    *,
    ...
    """Per-asset Donchian assignments. Missing assets omitted, not invented."""
    ...
    for asset, candles in by_canonical.items():
        series = donchian_position_series(
            candles,
            channel_n=channel_n,
...
class DonchianBreakoutVoter:
    """Look up a precomputed same-bar Donchian position."""
    ...
    def evaluate(self, candles: tuple[Candle, ...], regime: Regime) -> StrategySignal:
        ...
        if value is None:
            return StrategySignal(
                ...
                confidence=0.0,
                regime=regime,
                rationale="donchian: no same-bar assignment (warmup / skipped)",
            )
        side: Side | None = None
        if value > 0:
            side = Side.BUY
        ...
        return StrategySignal(
            ...
            regime=regime,
            rationale=f"donchian assignment={value:+.0f}",
        )
...
def _book_label(*, channel_n: int, long_short: bool, use_atr: bool) -> str:
    book = "long/short symmetric" if long_short else "long-only"
    if use_atr:
        return (
            f"{book} Donchian {channel_n}d + ATR({DONCHIAN_ATR_PERIOD})"
            f"×{DONCHIAN_ATR_MULTIPLIER:g} buffer"
        )
    return f"{book} Donchian {channel_n}d (exit `{EXIT_RULE}`)"


def donchian_candidates(
    ...
    *,
    ...
    source = histories or {}
    for candidate_id, channel_n, long_short, use_atr in DONCHIAN_CATALOG:
        ...
        out.append(
            SearchCandidate(
                ...
                family="donchian",
                label=_book_label(channel_n=channel_n, long_short=long_short, use_atr=use_atr),
                params={
                    "channel_n": channel_n,
                    "long_short": long_short,
                    "use_atr": use_atr,
                    "atr_period": DONCHIAN_ATR_PERIOD if use_atr else None,
                    "atr_multiplier": DONCHIAN_ATR_MULTIPLIER if use_atr else None,
                    "exit_rule": EXIT_RULE,
                    "strategy_id": candidate_id,
                    "multi_asset_gate": MULTI_ASSET_GATE_RULE,
                },
                strategy=DonchianBreakoutVoter(
                    strategy_id=candidate_id,
        ...
        skipped.append(
            {
                ...
                "candidate_id": candidate_id,
                "reason": (
                    f"{_book_label(channel_n=channel_n, long_short=long_short, use_atr=use_atr)}"
                    ": skipped — need venue-local OHLC with at least "
                    f"{channel_n + 1} bars"
                    + (f" and ATR({DONCHIAN_ATR_PERIOD}) warmup" if use_atr else "")
                    + ". Skip rather than invent or zero-fill closes."
                ),
            }
        )
    return skipped
...
def rank_donchian_passers(rows: list[DualPrintRow]) -> list[DualPrintRow]:
    """Frozen ranking key among Donchian dual-print passers. Control excluded."""
...
class DonchianBreakoutReport(BaseModel):
...
def _recommendation(
    *,
    ...
    lines.append(
        ...
        "or #108 EMA dual-prints. Do not re-run the #116 residual or "
        "#117 cross-sectional catalogs on the same windows. Do not "
        "invent PIT basis for carry."
    )
    return "\n".join(lines)


def run_donchian_breakout_search(
    ...
    *,
    ...
    catalog = candidates if candidates is not None else donchian_candidates(kraken)
    skipped = skipped_donchian_families(kraken)
    ...
    if binance_meta.available:
        if candidates is not None:
            ...
            binance_catalog = donchian_candidates(remapped)
    ...
    return DonchianBreakoutReport(
        ...
        catalog_note=DONCHIAN_CATALOG_NOTE,
        fee_bps=fee_bps,
        slippage_bps=slippage_bps,
        train_size=train_size,
        test_size=test_size,
...
def render_donchian_breakout_markdown(
    report: DonchianBreakoutReport,
) -> str:
    dual_rows = [
    ...
    lines: list[str] = [
        ...
        "",
        (
            "On each asset at bar t, the channel is the max high and min "
            "low of the prior N bars `[t-N, t)`. Bar t's high/low never "
            f"enter that level. Exit rule `{EXIT_RULE}`: long-only is "
            "flat after close[t] < prior low; long/short is short after "
            "that band. Between the bands the previous side is held "
            "(start flat). ATR-buffered long-only names add frozen "
            f"Wilder ATR({DONCHIAN_ATR_PERIOD}) × "
            f"{DONCHIAN_ATR_MULTIPLIER:g} computed on bars through t-1. "
            "Decision uses close[t]; the shared backtest fills at bar "
            "t+1 open. A missing series is skipped, never zero-filled."
        ),
        "",
    ...
    lines.extend(
        [
            ...
            "",
            (
                f"Frozen ranking key: `{report.ranking_key}`. Only Donchian "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "These Donchian names cleared #96+A+B+C on the Kraken "
    ...
    lines.extend(
        [
            ...
            "",
            (
                "These Donchian names cleared #96+A+B+C on the Binance.US "
    ...
    family_by_id = {item: ("control" if item in CONTROL_IDS else "donchian") for item in CORE_IDS}
    ...
    lines.extend(
        [
            ...
            (
                ...
                "catalog, or the #117 cross-sectional catalog on the same "
                "windows. Do not invent carry basis."
            ),
            "",
        ]
    )
