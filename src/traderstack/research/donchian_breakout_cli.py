"""`traderstack-donchian-breakout`: Donchian dual-print.

Scores the frozen long-only / long-short / ATR-buffered catalog on the
Kraken primary 720-bar daily window and the #102 Binance.US older-720.
A name must combined-pass **both** prints. Ranking is Kraken mean
holdout excess among dual-print passers. Never flips
``PAPER_PROMOTE_*``. An empty passer set is success. Paper-only; no
live.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Donchian / channel-breakout dual-print: long-only or "
            "long/short on the prior N-day channel (N in {20, 55, 100}; "
            "optional ATR(14) buffer on lo 20/55). Must clear Kraken "
            "primary harder combined gates (#96+A+B+C on BTC+ETH; SOL "
            f"reported) AND the #102 Binance.US older-720 combined "
            f"gates. Ranking key (frozen): {RANKING_KEY}. Does not flip "
            "PAPER_PROMOTE_* flags. An empty dual-print set is success. "
            "Not an EMA, residual, or XS-momentum reprint. No live."
        )
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/donchian_breakout.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/donchian-breakout.md"),
    )
...
def run(
    ...
    *,
    ...
    report = run_donchian_breakout_search(
        kraken_histories,
        binance_histories,
        fee_bps=fee_bps,
    ...
    markdown = render_donchian_breakout_markdown(report)
