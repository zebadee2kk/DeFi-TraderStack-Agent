"""`traderstack-lead-lag`: BTC→ETH lead-lag dual-print.

Scores the frozen ETH-follows-lagged-BTC catalog (plus a small BTC
mirror set) on the Kraken primary 720-bar daily window and the #102
Binance.US older-720. A name must combined-pass **both** prints.
Ranking is Kraken mean holdout excess among dual-print passers.
Never flips ``PAPER_PROMOTE_*``. An empty passer set is success.
Paper-only; no live.
"""
...
DEFAULT_KRAKEN_SYMBOLS = ("BTC/USD", "ETH/USD")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "BTC→ETH lead-lag dual-print: ETH follows or fades lagged "
            "BTC L-day return (L in {1,2,3,5} follow-lo; {1,2,3} "
            "follow-ls / fade-lo; BTC-follows-ETH mirror lo {1,2}). "
            "Not same-bar residual z-score (#116). Must clear Kraken "
            "primary harder combined gates (#96+A+B+C on both BTC and "
            f"ETH legs) AND the #102 Binance.US older-720 combined "
            f"gates. Ranking key (frozen): {RANKING_KEY}. Does not flip "
            "PAPER_PROMOTE_* flags. An empty dual-print set is success. "
            "Not an EMA, residual, XS, Donchian, TSMOM, Bollinger, or "
            "calendar reprint. No live."
        )
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/lead_lag.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/lead-lag.md"),
    )
...
def run(
    ...
    *,
    ...
    notes.append("universe=" + ",".join(UNIVERSE) + " (BTC+ETH pair; unpaired days skipped)")
    ...
    markdown = render_lead_lag_markdown(report)
