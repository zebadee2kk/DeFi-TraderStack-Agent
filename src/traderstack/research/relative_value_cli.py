"""`traderstack-relative-value`: BTC−ETH residual dual-print search.

Scores the frozen fade/follow |z| catalog on the Kraken primary 720-bar
daily window and the #102 Binance.US older-720. A name must
combined-pass **both** prints. Ranking is Kraken mean holdout excess
among dual-print passers. Never flips ``PAPER_PROMOTE_*``. An empty
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "BTC−ETH relative-value residual dual-print: fade/follow "
            "daily r_BTC − r_ETH at frozen |z| thresholds. Must clear "
            "Kraken primary harder combined gates (#96+A+B+C) AND the "
            f"#102 Binance.US older-720 combined gates. Ranking key "
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/relative_value.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/btc-eth-relative-value.md"),
    )
...
def run(
    ...
    *,
    ...
    report = run_relative_value_search(
        kraken_histories,
        binance_histories,
        fee_bps=fee_bps,
    ...
    markdown = render_relative_value_markdown(report)
    ...
    if args.stdout_md:
        ...
        print(
            ...
            f"residual_points={report.residual_points}; "
