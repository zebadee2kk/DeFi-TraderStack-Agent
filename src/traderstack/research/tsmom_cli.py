"""`traderstack-tsmom`: time-series momentum dual-print.

Scores the frozen long-only / long-short own-asset trailing-return
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Time-series momentum dual-print: long-only or long/short "
            "on each asset's own trailing N-day close-to-close return "
            "(N in {21, 63, 126, 252}). Must clear Kraken primary "
            "harder combined gates (#96+A+B+C on BTC+ETH; SOL "
            ...
            "Not an EMA, residual, XS-momentum, or Donchian reprint. "
            "No live."
        )
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/tsmom.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/tsmom.md"),
    )
...
def run(
    ...
    *,
    ...
    markdown = render_tsmom_markdown(report)
