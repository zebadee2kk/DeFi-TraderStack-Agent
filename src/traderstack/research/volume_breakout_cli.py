"""`traderstack-volume-breakout`: volume-confirmed breakout dual-print.

Scores the frozen long-only / long-short / volume-surge catalog on
the Kraken primary 720-bar daily window and the #102 Binance.US
older-720. Every promote-eligible name requires a volume gate
(not a Donchian N retune). A name must combined-pass **both**
prints. Ranking is Kraken mean holdout excess among dual-print
passers. Never flips ``PAPER_PROMOTE_*``. An empty passer set is
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Volume-confirmed breakout dual-print: long-only or "
            "long/short on the prior N-day channel **and** a volume "
            "gate (N×mult in {20x1.5, 55x1.5, 20x2}; V=20 SMA through "
            "t−1), plus a small volume-surge set. Not a Donchian N "
            "retune (#118). Must clear Kraken primary harder combined "
            "gates (#96+A+B+C on BTC+ETH; SOL reported) AND the #102 "
            f"Binance.US older-720 combined gates. Ranking key "
            f"(frozen): {RANKING_KEY}. Does not flip PAPER_PROMOTE_* "
            "flags. An empty dual-print set is success. Not an EMA, "
            "residual, XS, Donchian, TSMOM, Bollinger, calendar, or "
            "lead-lag reprint. No live."
        )
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("var/ops/volume_breakout.json"),
    )
    parser.add_argument(
        "--output-md",
        type=Path,
        default=Path("docs/artifacts/strategy-search/volume-breakout.md"),
    )
...
def run(
    ...
    *,
    ...
    report = run_volume_breakout_search(
        kraken_histories,
        binance_histories,
        fee_bps=fee_bps,
    ...
    markdown = render_volume_breakout_markdown(report)
